...
function bind() {
  ...
  $("#btnVectors")?.addEventListener("click", () => {
    ...
    const rs = $("#roomStatus");
    void probeSignal().then((live) => {
      if (!rs) return;
      if (state.room.id) {
        rs.textContent = `${state.room.id} · ${state.room.connected ? "connected" : "waiting"}`;
        return;
      }
      rs.textContent = live ? t(state, "signalLive") : t(state, "signalLocal");
    });
  ...
  $("#btnCreateRoom")?.addEventListener("click", async () => {
    try {
      ...
      const el = $("#roomStatus");
      if (el) {
        el.textContent = r.signalOk
          ? `${r.id} · ${t(state, "signalLiveShort")}`
          : `${r.id} · ${t(state, "signalLocalShort")}`;
      }
    } catch (_) {
  ...
  $("#btnJoinRoom")?.addEventListener("click", async () => {
    try {
      ...
      const el = $("#roomStatus");
      if (el) {
        el.textContent = isSignalLive()
          ? `${id.toUpperCase()} · ${t(state, "signalLiveShort")}`
          : `${id.toUpperCase()} · ${t(state, "signalLocalShort")}`;
      }
    } catch (_) {
      toast(t(state, "toastRoomFail"));
    }
  ...
  try {
    ...
    if (room) {
      if (room.includes("@") || room.includes(",")) {
        ...
        openAr(false);
        joinRoom(room, onRoomMessage, () => {})
          .then(() => toast(t(state, "toastRoomJoin")))
          .catch(() => toast(t(state, "toastRoomFail")));
      }
    }
