...
export function activeLayoutCards() {
  if (Array.isArray(state.layout) && state.layout.length) return state.layout;
  return demoLayout();
}

export function ensureMutableLayout() {
  if (!Array.isArray(state.layout) || !state.layout.length) {
    state.layout = demoLayout().map((card) => ({ ...card }));
  }
  return state.layout;
}
...
function renderCard(feed, p, i, handles, ghost) {
  const stamped = !ghost && handles.length ? handles[i % handles.length] : "";
  const handle = stamped || p.handle || "@local";
  const name = stamped ? handle.replace(/^@/, "") : p.name || handle.replace(/^@/, "");
  const plane = p.plane || (i % 3 === 0 ? "near" : i % 3 === 1 ? "mid" : "far");
  const card = document.createElement("article");
  card.className = `post-card ${plane}`;
  if (ghost || p.ghost) card.classList.add("is-ghost");
  if (p.pinned) card.classList.add("is-pinned");
  if (p.source === "note") card.classList.add("is-note");
  card.dataset.id = String(p.id || "card-" + i);
  ...
  card.dataset.source = p.source || (ghost ? "ghost" : "demo");
  card.tabIndex = 0;
  ...
  const insightHtml = state.settings.showInsights
    ...
    : "";
  const pinHtml = p.pinned ? `<span class="pin-tag">PINNED</span>` : "";
  const peerHtml = ghost || p.ghost ? `<span class="peer-tag">peer</span>` : "";
  ...
  card.innerHTML = `
      ...
      <div class="post-stats">
        <span>${escapeHtml(p.reposts ?? 0)} reposts</span>
        <span>${escapeHtml(p.likes ?? 0)} likes</span>
      </div>
      <span class="react-burst" hidden></span>
      ${insightHtml}
    `;
...
export function buildPosts() {
  ...
  activeLayoutCards().forEach((p, i) => renderCard(feed, p, i, handles, false));
  (state.ghosts || []).forEach((p, i) => renderCard(feed, p, i, [], true));
