...
export function setRoomOpenHandler(fn) {
  onDcOpen = typeof fn === "function" ? fn : null;
}

function blankRoom() {
  return { id: null, role: null, peers: [], connected: false, channel: "off", peerCount: 0 };
}
...
export function send(obj) {
  ...
  if (bc) {
    ...
    if (!viaDc && state.room.channel !== "dc") state.room.channel = "bc";
  }
}
...
export async function createRoom(onMessage, onStatus) {
  await leaveRoom();
  const id = code6();
  state.room = { id, role: "host", peers: [], connected: false, channel: "waiting", peerCount: 0 };
...
export async function joinRoom(id, onMessage, onStatus) {
  ...
  state.room = { id, role: "guest", peers: [], connected: false, channel: "waiting", peerCount: 0 };
  peerId = Math.random().toString(36).slice(2, 10);
  pollSince = 0;
...
async function handleSignal(msg, onMessage, onStatus, isHost) {
  ...
  if (msg.type === "dc" && msg.payload) {
    if (state.room.channel !== "dc") state.room.channel = "bc";
