# X-Ray 4.0.0 - Grounded Reality

**Your posts. Your room. On this device.**

Spatial posts over live camera, on this device. Import a feed you already have, save the layout as a Place, and share anchors with another phone. This host does not fetch X. Camera frames and captures are not uploaded.
...
- **License:** MIT
- **Version:** 4.0.0 (`VERSION` + [CHANGELOG.md](./CHANGELOG.md))

## What's new in 4.0.0

Grounded Reality.

- Place Memory keeps orientation-relative pins on this device. Not a 3D scan.
- Feed Bay imports JSON, NDJSON, markdown, or an X archive file you already have.
- Shared Reality sends anchors, ghost cards, and reactions while the data channel is up. The signaling worker still stores SDP and ICE only.
- Pins, notes, and a wider Local Pulse. WebXR hit-test is optional.

## What's new in 3.2.0

Local Pulse: an on-device diagnostics strip (version, cache, camera permission). Camera frames still never leave this browser.
...
- Imported posts and place memory

What can leave the browser:

- Shared Reality: SDP, ICE, presence, handle labels, and small v4 anchor JSON or reactions while a room is active (never video). Anchors sync only while the data channel is up. NAT may block rooms. No TURN server.
...
| `/?place=ID` | Open a saved place on this device |
| `/?places=1` | Open Places |
| `/?feed=owned` | Show the imported feed |
| `/?import=1` | Open Feed Bay |
...
public/
  ...
  assets/app/          # ES modules
    main-v400.js       # current entry
    main-v302.js       # 3.2 reference
    places.js
    owned-feed.js
    share-protocol.js
...
| Shared Reality | WebRTC data channel + STUN. Anchors sync only while that channel is up. NAT may block rooms. No TURN server. |
...
| R | Re-anchor (writes the active place) |
| Space | Still capture (when focus on body) |
| P | Pin the focused card |
| L | Cycle Places |
| N | Note |
| F | Feed Bay |
| I | Insight on the focused card |

## Credits

Concept and experimental product direction by [@suddenlyjon](https://x.com/suddenlyjon).  
Network host, v3 Anchored Reality, and v4 Grounded Reality: Pitchfork-and-Torch.
